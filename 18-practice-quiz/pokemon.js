...
//     order: pokemon.order,
//     name: pokemon.name,
//     abilities: pokemon.abilities.sort((a,b) => a.slot > b.slot).map(abilityObject => abilityObject.ability.name),
//     moves: pokemon.moves.filter(moveObj => moveObj.version_group_details[0].version_group.name === "red-blue").map(move => move.move.name).slice(0, 10),
//     stats: pokemon.stats.sort((a, b) => a.base_stat < b.base_stat).map(statObject => ({ value: statObject.base_stat, name: statObject.stat.name})),
...
//   let i = 1, oldPokemons = []
//   while (i < 151) { oldPokemons.push(++i) }
//
//   oldPokemons.reduce((promise, pokemon) => {
//     return promise
//       .then(emptyValue => fetch(`http://pokeapi.co/api/v2/pokemon/${pokemon}/`))
//       .then(response => response.json())
//       .then(transformPokemon)
//       .then(newPokemonObj => newPokemons.push(newPokemonObj));
//   }, Promise.resolve()).then(() => console.log(newPokemons));
// }
